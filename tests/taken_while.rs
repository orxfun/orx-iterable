...
mod object_safe {
    ...
    fn obj_taken_while() {
        let a = vec![1, 3, 7, 2, 8];
        obj_test_it(vec![], &a.taken_while(|x| **x > 100).copied());

        let a = vec![1, 3, 7, 2, 8];
        obj_test_it(vec![1, 3], &a.taken_while(|x| **x < 5).copied());

        let a = vec![1, 3, 7, 2, 8];
        obj_test_it(vec![1, 3, 7, 2, 8], &a.taken_while(|x| **x < 100).copied());
    }

    #[test]
    fn obj_taken_while_mut() {
        let mut a = vec![1, 3, 7, 2, 8];
        for x in a.taken_while_mut(|x| *x > 100).iter_mut() {
            *x += 10;
        }
        ...
        obj_test_it(vec![11, 13, 7, 2, 8], &a.copied());
    ...
    fn obj_into_taken_while() {
        ...
        obj_test_it(vec![], &a.copied()); // now they are above 5!
